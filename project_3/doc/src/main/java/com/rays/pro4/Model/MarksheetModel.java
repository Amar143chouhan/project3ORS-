...
public class MarksheetModel {
	...
	public List search(MarksheetBean bean, int pageNo, int pageSize) throws ApplicationException {
		...
		if (bean != null) {
			...
			System.out.println("tcheck" + bean.getTcheck());
			...
			if(bean.getTcheck()!= null && bean.getTotal()>0) {
				System.out.println("hello i am in");
				
				if (bean.getTcheck().equalsIgnoreCase("greater")) {
					sql.append(" AND (physics+chemistry+maths) >" + bean.getTotal());
				}
				if (bean.getTcheck().equalsIgnoreCase("less")) {
					sql.append(" AND (physics+chemistry+maths) <" + bean.getTotal());
				}
				if (bean.getTcheck().equalsIgnoreCase("equal")) {
					sql.append(" AND (physics+chemistry+maths) =" + bean.getTotal());
				}
			}
			/*
			 * if(bean.getTotal()>0) { sql.append(" AND (physics+chemistry+maths) =" +
			 * bean.getTotal()); }
			 */
			if (bean.getId() > 0) {
	...
	public List list(int pageNo, int pageSize) throws ApplicationException {
		...
		System.out.println("Model  list return");
		return list;
